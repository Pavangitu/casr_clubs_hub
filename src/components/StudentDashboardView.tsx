...
export const StudentDashboardView: React.FC<StudentDashboardViewProps> = ({ 
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'clubs' | 'events' | 'committee' | 'about'>('dashboard');
  ...
  const [showCreditTrackModal, setShowCreditTrackModal] = useState(false);
  ...
  // Flexible registration number matching (handles spaces/hyphens/casing)
  ...
  const matchedLiveStudent = Array.isArray(allStudents)
    ? (allStudents.find((s) => matchReg(s?.registrationNumber, student?.registrationNumber)) as any)
    : null;
  ...
  // Dynamically calculate unique days attended from recent history or live Google Sheet scan logs
  const liveHistory = matchedLiveStudent?.recentHistory || [];

  const uniqueDaysSet = new Set(
    liveHistory
      .filter((r: any) => r && r.status !== 'ABSENT')
      .map((r: any) => (r.date || '').split(' ')[0].trim())
      .filter(Boolean)
  );

  const daysAttended = uniqueDaysSet.size > 0
    ? uniqueDaysSet.size
    : (matchedLiveStudent
        ? (matchedLiveStudent.insCount || matchedLiveStudent.eventsAttendedCount || student.classesAttended)
        : student.classesAttended);

  // Dynamically calculate total hours logged from check-in/check-out session durations in Google Sheet
  const hoursAttended = (matchedLiveStudent && matchedLiveStudent.completedHours !== undefined && matchedLiveStudent.completedHours > 0)
    ? Number(matchedLiveStudent.completedHours.toFixed(1))
    : (liveHistory.length > 0
        ? parseFloat(liveHistory.filter((r: any) => r && r.status !== 'ABSENT').reduce((sum: number, r: any) => sum + (r.durationHours || 1.5), 0).toFixed(1))
  ...
  const studentProfileForHistory: StudentProfile = {
    ...
    creditsEarned: student.creditsEarned !== undefined ? student.creditsEarned : 0,
    requiredHours: 120,
  ...
  return (
    <div className="min-h-screen transition-colors duration-300 font-sans relative overflow-x-hidden flex flex-col justify-between">
      ...
      <nav className="fixed top-4 left-1/2 -translate-x-1/2 w-[92%] max-w-7xl rounded-full border border-amber-500/20 dark:border-amber-500/30 bg-white/95 dark:bg-[#09090b]/90 backdrop-blur-[40px] shadow-[0_8px_32px_0_rgba(217,119,6,0.15)] dark:shadow-[0_8px_32px_0_rgba(0,0,0,0.6)] flex justify-between items-center px-4 md:px-8 py-3 z-50 transition-all duration-300">
        {/* Brand Logo & Name */}
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-full bg-amber-500/10 flex items-center justify-center border border-amber-500/30 p-1">
            <img
          ...
          <span className="text-base md:text-xl font-extrabold bg-gradient-to-r from-amber-400 via-yellow-300 to-amber-500 bg-clip-text text-transparent">
            CaSR Student Portal
          </span>
        </div>

        {/* Student Nav Tabs (Matching Admin Navbar Tab Pattern) */}
        <div className="flex items-center gap-1 sm:gap-1.5 p-1 bg-slate-200/80 dark:bg-zinc-900/90 border border-slate-300 dark:border-amber-500/20 rounded-full overflow-x-auto">
          <button
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          {toggleTheme && (
            <button
              onClick={toggleTheme}
              className="p-2 rounded-full text-slate-700 dark:text-gray-300 hover:text-amber-600 dark:hover:text-amber-400 hover:bg-black/5 dark:hover:bg-white/10 transition-colors"
          ...
          <button
            onClick={onLogout}
            className="flex items-center gap-1.5 px-3 md:px-4 py-1.5 rounded-full bg-amber-500/10 hover:bg-amber-500/20 text-amber-700 dark:text-amber-400 text-xs md:text-sm font-bold border border-amber-500/30 transition-all shadow-sm"
          >
      ...
      <main className="relative z-10 pt-24 pb-12 px-4 md:px-8 max-w-7xl mx-auto flex-1 w-full space-y-6">
        ...
        <div className="glass-card p-5 md:p-6 rounded-3xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xl border border-amber-500/20">
          <div className="flex items-center gap-4">
            <div>
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-0.5 rounded-full text-[11px] font-extrabold bg-amber-500/10 text-amber-700 dark:text-amber-300 border border-amber-500/30">
                  Student Portal
                </span>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-2 px-3.5 py-2 rounded-xl bg-amber-500/10 border border-amber-500/30 text-xs text-amber-700 dark:text-amber-300 font-extrabold">
        ...
        {activeTab === 'clubs' ? (
          ...
          <motion.div
            ...
            transition={{ duration: 0.3 }}
          >
            <AboutView onNavigateToClubs={() => setActiveTab('clubs')} />
          </motion.div>
        ) : (
          <>
            ...
            <motion.div
              ...
              className="glass-card rounded-3xl p-6 shadow-xl border border-amber-500/20"
            >
              <div className="flex items-center gap-2.5 mb-5 text-amber-600 dark:text-amber-400 font-extrabold text-xs uppercase tracking-wider">
                <span className="w-8 h-8 rounded-xl glass-neo-icon text-amber-500 dark:text-amber-400">
                  <GraduationCap className="w-4 h-4" />
                </span>
                Academic Profile Information
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 gap-4">
                <div className="p-4 bg-slate-100 dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-amber-500/20 shadow-sm flex flex-col justify-between">
                ...
                <div className="p-4 bg-slate-100 dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-amber-500/20 shadow-sm flex flex-col justify-between">
                  <span className="text-xs text-slate-758 dark:text-slate-300 font-bold flex items-center justify-between gap-2 mb-2">
                    ...
                    <span className="relative flex h-2 w-2" title="Live Synced with Google Sheet">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-amber-400 opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-2 w-2 bg-amber-500"></span>
                    </span>
                  </span>
                  <p className="text-sm font-extrabold text-slate-950 dark:text-white">
                    {matchedLiveStudent?.semesterYear || student.semesterYear || (student.semester ? `Semester ${student.semester}` : '3rd year')}
                  </p>
                </div>
                <div className="p-4 bg-slate-100 dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-amber-500/20 shadow-sm flex flex-col justify-between">
            ...
            <motion.div
              ...
              style={{ transformStyle: 'preserve-3d', perspective: 1200 }}
              className="glass-card bg-gradient-to-r from-amber-500/15 via-yellow-500/10 to-amber-600/15 border border-amber-500/30 rounded-3xl p-5 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-4"
            >
              <div className="flex items-center gap-4">
                <div className="p-3 glass-neo-icon rounded-2xl text-amber-500 dark:text-amber-400 shrink-0">
                  <Shield className="w-7 h-7" />
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-extrabold uppercase tracking-wider px-2 py-0.5 rounded bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/30">
              ...
              <button
                onClick={() => setActiveTab('committee')}
                className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-600 hover:to-yellow-600 text-white font-extrabold text-xs transition-all shadow-md shadow-amber-500/20 whitespace-nowrap flex items-center gap-2 shrink-0 cursor-pointer"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="relative flex h-2 w-2" title="Real-Time Google Sheets Sync Active">
              ...
              {/* Earned Member Credits & Faculty Track Card */}
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.15 }}
                whileHover={{ scale: 1.02 }}
                className="glass-card rounded-3xl p-6 shadow-xl relative overflow-hidden bg-gradient-to-br from-amber-500/10 via-slate-900/90 to-amber-950/20 border border-amber-500/30"
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-black text-amber-400 uppercase tracking-widest flex items-center gap-1.5">
                      <Award className="w-4 h-4 text-amber-400" /> MY EARNED CREDITS
                    </span>
                  </div>
                ...
                <div className="flex items-baseline justify-between mb-3">
                  <div className="flex items-baseline gap-2">
                    <span className="text-4xl md:text-5xl font-black text-amber-400 font-mono tracking-tight">
                      {matchedLiveStudent?.creditsEarned !== undefined ? matchedLiveStudent.creditsEarned : (student.creditsEarned !== undefined ? student.creditsEarned : 0)}
                    </span>
                    <span className="text-sm font-bold text-slate-300">Total Credits</span>
                  </div>

                  <button
                    onClick={() => setShowCreditTrackModal(true)}
                    className="px-3 py-1.5 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-500/40 text-xs font-extrabold flex items-center gap-1.5 transition-all cursor-pointer"
                  >
                    <span>View Track Log</span>
                    <ChevronRight className="w-3.5 h-3.5" />
                  </button>
                </div>

                <p className="text-xs text-slate-400 mt-2 font-medium">
                  Faculty Lead awards & member credit history ({matchedLiveStudent?.creditLogs?.length || student.creditLogs?.length || 0} items logged).
                </p>
              </motion.div>
              ...
              <motion.div
                ...
                className="glass-card rounded-3xl p-6 shadow-xl relative overflow-hidden border border-amber-500/20"
              >
                <div className="flex items-center justify-between mb-4">
                  <span className="text-xs font-extrabold text-slate-800 dark:text-slate-200 uppercase tracking-wider">
                    Degree Target
                  </span>
                  <span className="px-2.5 py-1 rounded-full text-xs font-extrabold bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/30">
                    4-Year Goal
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2 mt-3">
                  <span className="px-2.5 py-0.5 rounded-md text-xs font-extrabold bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/30">
                    (120 hrs)
                  </span>
            ...
            <motion.div
              ...
              className="glass-card rounded-3xl p-6 shadow-xl relative overflow-hidden flex flex-col gap-6 border border-amber-500/20"
            >
              {/* Header */}
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <h2 className="text-xl font-extrabold text-slate-950 dark:text-white flex items-center gap-3 tracking-tight">
                  <span className="w-9 h-9 rounded-xl glass-neo-icon text-amber-500 dark:text-amber-400 shrink-0">
                    ...
                    <button
                      ...
                      className={`px-3.5 py-1.5 rounded-full text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${
                        selectedMonthStr === m
                          ...
                          : 'bg-slate-100 dark:bg-zinc-900 text-slate-700 dark:text-slate-400 border border-slate-200 dark:border-amber-500/20 hover:bg-slate-200 dark:hover:bg-zinc-800'
                      }`}
                    >
                      {m}
                    </button>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-amber-500/10 p-3 rounded-2xl border border-amber-500/20 flex flex-col justify-center">
                  <span className="text-[10px] font-extrabold text-slate-750 dark:text-slate-400 uppercase tracking-wide">Classes Logged</span>
                  <span className="text-xl font-black text-amber-500 dark:text-amber-400 font-mono mt-0.5">
                    {monthLogs.length}
                  </span>
                </div>
                <div className="bg-amber-500/10 p-3 rounded-2xl border border-amber-500/20 flex flex-col justify-center">
              ...
              {/* Graph Container */}
              <div className="relative bg-slate-100/90 dark:bg-zinc-900/90 p-4 rounded-3xl border border-slate-200/90 dark:border-amber-500/20 shadow-inner min-h-[220px]">
                {/* SVG Graph */}
                  ...
                  <div className="absolute inset-x-4 top-4 bottom-10 flex flex-col justify-between pointer-events-none text-[9px] font-bold text-slate-400 font-mono">
                    {[maxDayHours, maxDayHours / 2, 0].map((h, i) => (
                      <div key={i} className="flex items-center gap-2 w-full">
                        ...
                        <div className="w-full border-b border-dashed border-slate-200 dark:border-zinc-800" />
                      </div>
                    ))}
                  </div>
                        ...
                        >
                          ...
                          <div
                            className={`w-full rounded-t-lg transition-all duration-300 ${
                              isSelected 
                                ? 'bg-gradient-to-t from-amber-600 to-yellow-400 shadow-[0_0_12px_rgba(245,158,11,0.4)] scale-x-110' 
                                : isSelected === null && hasActivity
                                  ? 'bg-gradient-to-t from-amber-500/80 to-yellow-400/80 group-hover:from-amber-500 group-hover:to-yellow-400 shadow-[0_0_6px_rgba(245,158,11,0.1)]'
                                  : hasActivity
                                    ? 'bg-gradient-to-t from-amber-500/60 to-yellow-400/60 group-hover:from-amber-500/90 group-hover:to-yellow-400/90'
                                    : 'h-1.5 w-1.5 rounded-full bg-slate-350 dark:bg-zinc-800'
                            }`}
                ...
                <div className="flex justify-between text-[9px] font-bold text-slate-500 dark:text-slate-400 font-mono border-t border-slate-200 dark:border-zinc-800 pt-2 px-8">
                  <span>1st</span>
                  <span>10th</span>
                  <span>20th</span>
                ...
                {selectedDay !== null && dailyData[selectedDay - 1] ? (
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      {dailyData[selectedDay - 1].logs.map((log, lIdx) => (
                        <div 
                          key={lIdx}
                          className="bg-white/60 dark:bg-zinc-900/80 p-3 rounded-xl border border-slate-200 dark:border-amber-500/20 flex items-center justify-between gap-3"
                        >
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-lg bg-amber-500/10 text-amber-600 dark:text-amber-400 flex items-center justify-center font-bold text-[10px]">
                              {log.rawScanType || 'IN'}
                            </div>
                            <div>
                          ...
                          <div className="flex items-center gap-2 text-right">
                            <span className="text-[10px] font-extrabold font-mono bg-slate-100 dark:bg-zinc-900 px-2 py-0.5 rounded-md border border-slate-200 dark:border-amber-500/20">
                            ...
                            <span className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider ${
                              log.status === 'PRESENT'
                                ...
                                : 'bg-zinc-100 dark:bg-zinc-800 text-slate-800 dark:text-slate-300 border border-zinc-300 dark:border-zinc-700'
                            }`}>
                              {log.status}
                            </span>
                          </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {monthLogs.length > 0 ? (
                        ...
                        {monthLogs.map((log, lIdx) => {
                          ...
                          return (
                            <div 
                              ...
                              className="bg-white/40 dark:bg-zinc-900/50 p-2.5 rounded-xl border border-slate-200/60 dark:border-amber-500/20 hover:bg-slate-100/60 dark:hover:bg-zinc-800/80 transition-colors flex items-center justify-between gap-3 cursor-pointer"
                            >
                              <div className="flex items-center gap-2">
                      ...
                      <div className="bg-slate-100/40 dark:bg-zinc-900/40 p-4 rounded-xl border border-slate-200/50 dark:border-amber-500/20 text-center text-[11px] font-bold text-slate-500 dark:text-slate-400">
        ...
        <div className="p-4 rounded-2xl bg-amber-500/10 dark:bg-zinc-900 border border-amber-500/30 text-center text-xs text-amber-900 dark:text-amber-200 font-semibold flex items-center justify-center gap-2">
          <ShieldCheck className="w-4 h-4 text-amber-600 dark:text-amber-400 shrink-0" />
          <span>
            Authenticated Student Session for Registration No <strong className="text-slate-950 dark:text-white font-mono">{student.registrationNumber}</strong>. 
      ...
      {/* Earned Credit Track Modal for Student (Read-Only) */}
      {showCreditTrackModal && (
        <CreditTrackModal
          isOpen={showCreditTrackModal}
          onClose={() => setShowCreditTrackModal(false)}
          studentName={student.name}
          registrationNumber={student.registrationNumber}
          avatar={student.avatar}
          currentCredits={matchedLiveStudent?.creditsEarned !== undefined ? matchedLiveStudent.creditsEarned : (student.creditsEarned !== undefined ? student.creditsEarned : 0)}
          creditLogs={matchedLiveStudent?.creditLogs || student.creditLogs || []}
          isFaculty={false}
        />
      )}
